...
namespace GB {
	...
	template<>
	int CPU::LD<ByteReg, ByteReg>(ByteReg dest, ByteReg src)
	{
		mRegisters[dest] = mRegisters[src];
	}
	template<>
	int CPU::LD<ByteReg, Address>(ByteReg dest, Address src)
	{
		mRegisters[dest] = AddressBus::Read(src);
	}
